    ...
    astarfly: async function (bot, target, border1, border2, lastFlyTime, mute) {  //直接使用方塊座標
        ...
        async function astar(locNow, target, border1, border2) {
            ...
            if (end == null || end == undefined) {
                ...
                log(bot, false,'DEBUG','\x1b[31mA*未發現路徑\x1b[0m wait 50ms');
                await sleep(50);
            }
        ...
        if (!mute) log(bot, false,'DEBUG',`A*共計耗時\x1b[33m${astarEndT - astarStartT}\x1b[0m ms 距離${Math.round(distance * 10) / 10} M  速率 ${Math.round(distance * 10 / ((astarEndT - astarStartT) / 1000)) / 10}m/s`);
        // return lastFlyTime;      
        return 0
    }
}
