            ...
            while (build_cache.currentPalette < targetSch.palette.length) {
                ...
                for (let [chunkKey, indices] of spatialHash.entries()) {
                    ...
                    for (let i = 0; i < indices.length; i++) {
                        ...
                        // 分工邏輯：只處理屬於自己編號的方塊
                    if (cfg.worker_count > 1 && (currentPaletteBlocksIndexs[idx] % cfg.worker_count !== cfg.worker_id)) continue;
                ...
                for (let cP_dz = -4; cP_dz <= 4; cP_dz++) {
                    for (let cP_dx = -4; cP_dx <= 4; cP_dx++) {
                        for (let cP_dy = 5; cP_dy >= -3; cP_dy--) {
                            ...
                            // 分工邏輯：只處理屬於自己編號的方塊
                            if (cfg.worker_count > 1 && (currentPosPlace_d_Index % cfg.worker_count !== cfg.worker_id)) continue;
