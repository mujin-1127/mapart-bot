...
class Logger {
  ...
  module(moduleName) {
    return {
      info: (msg, meta) => this.info(`[${moduleName}] ${msg}`, meta),
      warn: (msg, meta) => this.warn(`[${moduleName}] ${msg}`, meta),
      error: (msg, meta) => this.error(`[${moduleName}] ${msg}`, meta),
      debug: (msg, meta) => this.debug(`[${moduleName}] ${msg}`, meta),
      log: (showInConsole, level, msg) => {
        if (showInConsole) this.log(level, `[${moduleName}] ${msg}`);
      }
    };
  }
}
