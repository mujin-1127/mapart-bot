...
class MapartPrinter extends BasePrinter {
    ...
    async logInventory(bot) {
        const items = bot.inventory.items();
        const itemStrings = items.map(item => `${item.name} x${item.count}`);
        
        let msg = `\n============= [${bot.username}] 目前背包內容 =============\n`;
        if (itemStrings.length === 0) {
            msg += `背包是空的\n`;
        } else {
            msg += itemStrings.join('\n') + '\n';
        }
        msg += `==============================================================\n`;
        
        // 輸出到日誌
        const logger = globalLogger.module('Inventory', bot.username);
        logger.info(msg);
    }
                        ...
                        for (let cP_dy = 5; cP_dy >= -3; cP_dy--) {
                            ...
                            if (dBlocksIndex !== -1 && currentPaletteBlocksState[dBlocksIndex] == 0 && Block_In_CD.indexOf(dBlocksIndex) == -1) {
                                ...
                                if (hold == null || hold.name != currentPaletteName) {
                                    ...
                                    if (allItems.length === 0) {
                                        ...
                                        while (waitTime < 1500) {
                                            await sleep(50);
                                            bot.updateHeldItem();
                                            if (bot.heldItem && bot.heldItem.name === currentPaletteName) break;
                                            waitTime += 50;
                                        }
                                    }
                                }
                                ...
                                if (consecutiveFailures >= 20) {
                                    this.log(bot, true, "WARN", `連續嘗試放置 ${consecutiveFailures} 次未偵測到成功，重新掃描背包並同步...`);
                                    consecutiveFailures = 0;
                                    await bot.closeWindow(bot.currentWindow).catch(() => {});
                                    await sleep(500);
                                    // 透過更新 heldItem 來強制同步一下
                                    bot.updateHeldItem();
                                }
                    ...
                    for (const bIdx of missingIndices) {
                        ...
                        if (!bot.heldItem || bot.heldItem.name !== expectedName) {
                            ...
                            if (invItem) {
                                ...
                                while (waitTime < 1500) {
                                    await sleep(50);
                                    bot.updateHeldItem();
                                    if (bot.heldItem && bot.heldItem.name === expectedName) break;
                                    waitTime += 50;
                                }
                            } else {
                        ...
                        if (bot.heldItem && bot.heldItem.name === expectedName) {
                            ...
                            if (!placedSuccess) {
                                consecutiveFailures++;
                                if (consecutiveFailures >= 20) {
                                    this.log(bot, true, "WARN", `連續放置失敗 ${consecutiveFailures} 次，強制重新掃描物品欄並檢查物資站...`);
                                    consecutiveFailures = 0; // 重置
                                    await bot.closeWindow(bot.currentWindow).catch(() => {});
                                    await sleep(500);
                                    // 這裡觸發一次物資檢查邏輯 (或是乾脆飛去補給站)
                                    // 簡單做法：強迫下一輪檢查物資
                                    const invItem = bot.inventory.items().find(item => item.name === expectedName);
                                    if (!invItem && stationConfig) {
                                        await station.restock(bot, stationConfig, [{ name: expectedName, count: 64 }]);
                                    }
                                }
                            }
                        }
