...
const { sleep, readConfig, saveConfig, v, hashConfig, clearInventory, moveToHotbar, syncHeldItem, calculateAvailableSpace } = require('../utils');
...
class BuildingPrinter extends BasePrinter {
    ...
    async build(task, bot, cfg, project, sharedState) {
        ...
        for (let i = 0; i < targetSch.Metadata.TotalVolume; i++) {
            let pId = targetSch.getBlockPIDByIndex(i);
            const paletteName = targetSch.palette[pId].Name;
            
            if (paletteName === "air" || paletteName === "cave_air" || this.BLOCK_SKIP_LIST.includes(paletteName)) continue;
            ...
            while (build_cache.currentLayer < build_cache.totalLayer) {
                ...
                if (!wheatherGetLayerPalette) {
                    ...
                    for (let i = startIndex; i <= endIndex; i++) {
                        ...
                        const paletteName = targetSch.palette[p_id].Name;
                        if (paletteName === "air" || paletteName === "cave_air" || this.BLOCK_SKIP_LIST.includes(paletteName)) continue;
                            ...
                            if (currentPaletteBlocksState[dBlocksIndex] == 0 && blockInCD.indexOf(dBlocksIndex) == -1) {
                                ...
                                if (this.checkBlock(bot.blockAt(dAbsolutePos), currentPalette) == 1) {
                                    ...
                                    if (hold?.name != currentPalette.Name) {
                                        const invItem = bot.inventory.items().find(item => item.name === currentPalette.Name);
                                        if (!invItem) {
                                            // 背包內無此物，觸發補貨
                                            let missingCountOfThisColor = 0;
                                            for (let m = 0; m < currentPaletteBlocksState.length; m++) {
                                                if (currentPaletteBlocksState[m] === 0) missingCountOfThisColor++;
                                            }
                                            let fetchCount = Math.min(64, missingCountOfThisColor + 2);
                                            await station.restock(bot, stationConfig, [{ name: currentPalette.Name, count: fetchCount }]);
                                            await pathfinder.astarfly(bot, selectBlockBotStandPos, null, null, null, true);
                                        } else {
                                            await moveToHotbar(bot, invItem.slot);
                                            await syncHeldItem(bot, currentPalette.Name);
                                        }
                                    }
                                    ...
                                    // 檢查是否放置成功
                                    await sleep(150);
                                    const placedBlock = bot.blockAt(dAbsolutePos);
                                    if (placedBlock && this.checkBlock(placedBlock, currentPalette) === 1) {
                                        this.consecutiveFailures = 0;
                                ...
                                if (this.consecutiveFailures >= 100) {
                                    this.log(bot, true, "WARN", "連續嘗試放置 100 次未偵測到成功，正在重啟機器人...");
                ...
                if (selectBlockIndex === -1) {
                    ...
                    if (hasUnfinished && blockInCD.length > 0) {
                        await sleep(200);
                        continue;
                    }
                ...
                if (missingIndices.length > 0) {
                    ...
                    for (const bIdx of missingIndices) {
                        ...
                        if (!hold || hold.name !== palette.Name) {
                            ...
                            if (invItem) {
                                await moveToHotbar(bot, invItem.slot);
                                await syncHeldItem(bot, palette.Name);
                            } else {
                                let missingCountOfThisColor = 0;
                                for (let mIdx of missingIndices) {
                                    if (targetSch.palette[targetSch.getBlockPIDByIndex(mIdx)].Name === palette.Name) {
                                        missingCountOfThisColor++;
                                    }
                                }
                                // 最少拿 1 組(64)以防萬一，但如果缺的很少就只拿剛剛好的數量(加點緩衝)
                                // 並且受限於背包空間
                                const avail = calculateAvailableSpace(bot, palette.Name);
                                let fetchCount = Math.min(64, missingCountOfThisColor + 2);
                                if (fetchCount > avail) fetchCount = Math.max(0, avail);

                                if (fetchCount > 0) {
                                    await station.restock(bot, stationConfig, [{ name: palette.Name, count: fetchCount }]);
                                    await pathfinder.astarfly(bot, absPos.offset(0, 2, 0), null, null, null, true);
                                } else {
                                    this.log(bot, true, "WARN", `背包已滿，無法補給 ${palette.Name}`);
                                }
                            }
                        }
                        ...
                        if (bot.heldItem && bot.heldItem.name === palette.Name) {
                            ...
                            while (attempt < 3) {
                                ...
                                if (checkCurrent && checkCurrent.name !== 'air' && checkCurrent.name !== 'cave_air' && this.checkBlock(checkCurrent, palette) !== 1) {
                                    try {
