    ...
    async build(task, bot, cfg, project, sharedState) {
        ...
        for (let i = 0; i < targetSch.Metadata.TotalVolume; i++) {
            ...
            if (cfg.workRegion) {
                ...
                const absX = rel.x + cfg.schematic.placementPoint_x;
                const absZ = rel.z + cfg.schematic.placementPoint_z;
                if (absX < cfg.workRegion.minX || absX > cfg.workRegion.maxX ||
                    absZ < cfg.workRegion.minZ || absZ > cfg.workRegion.maxZ) {
                    continue;
                }
            }
            ...
            while (build_cache.currentLayer < build_cache.totalLayer) {
                ...
                if (!wheatherGetLayerPalette) {
                    ...
                    for (let i = startIndex; i <= endIndex; i++) {
                        ...
                        // 分工邏輯
                        if (cfg.worker_count > 1) {
                            const ignoreAirIdx = globalToIgnoreAirIndex.get(i);
                ...
                for (let cP_dz = -4; cP_dz <= 4; cP_dz++) {
                    for (let cP_dx = -4; cP_dx <= 4; cP_dx++) {
                        ...
                        // 分工邏輯
                        if (cfg.worker_count > 1) {
                            const ignoreAirIdx = globalToIgnoreAirIndex.get(currentPosIndex);
                            if (ignoreAirIdx === undefined || (ignoreAirIdx % cfg.worker_count !== cfg.worker_id)) continue;
                        }
