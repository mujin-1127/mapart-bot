...
const log = (bot, showInConsole, level, msg) => {
...
async function model_mapart_build(task, bot, cfg, project) {
    ...
    let crt_cfg_hash = hashConfig(cfg.schematic)
    ...
    if (cfg.materialsMode == 'station') {
        ...
        log(bot, true,"INFO",`材料站 ${stationConfig.stationName} 加載成功`)
    }
    let targetSch 
    if(project){
        targetSch = project
    ...
    log(bot, true,"INFO",`開始建造 ${targetSch.Metadata.Name}`)
    ...
    mapartBuild: while (build_cache.currentPalette < targetSch.palette.length) {
        ...
        if (!wheatherGetPalette) {
            ...
            //if(debug_enable) 
            log(bot, false, 'INFO', `\x1b[32m當前材料:\x1b[0m ${currentPaletteName} ${build_cache.currentPalette + 1}/${targetSch.palette.length}`)
        ...
        */
                    ...
                    if (currentPaletteBlocksState[dBlocksIndex] == 0 && Block_In_CD.indexOf(dBlocksIndex) == -1) {
                        ...
                        if (hold == null || hold.name != currentPaletteName) {
                            ...
                            if (findMaterialSlot == -1) { //背包也沒有 回去補
                                ...
                                if (!mp) log(bot, false, 'DEBUG', '空')
                                if(debug_enable){
                                    log(bot, false, 'DEBUG', `當前背包物品`)
                                    for (const i in mp) {
                                        log(bot, false, 'DEBUG', `${i.slice(0, 16).toString().padEnd(16)} ${mp[i]}`)
                                    }
                                }
                                ...
                                if (debug_enable) log(bot, false, 'INFO', `\x1b[33m切換物品欄\x1b[0m ${findMaterialSlot} <-> 手`)
                                await bot.simpleClick.leftMouse(44)
                                await bot.simpleClick.leftMouse(findMaterialSlot)
                                await bot.simpleClick.leftMouse(44)
                                await sleep(80)
                                bot.updateHeldItem();
                                if (!bot.heldItem || bot.heldItem.name !== currentPaletteName) {
                                    log(bot, false, 'WARN', '切換後手上仍無材料，本輪跳過並重試')
                                    continue
                                }
                            }
                        ...
                        //     placeRateLimit--;
                        // }, 250)
                        if (debug_enable) log(bot, false, 'DEBUG', `\x1b[32m放置\x1b[0m ${currentPaletteName} (${build_cache.currentPalette}) 於 a${dAbsolutePos} r${dRelativePos} dBI:${dBlocksIndex} r_i:${currentPosPlace_d_Index} ${targetSch.getBlockByIndex(currentPosPlace_d_Index).Name}`)
                        //眼
        ...
        // FIND NEXT 優化版：直接從過濾後的列表尋找未完成且最近的點
        let deubg_startFNext = Date.now();
        let lastBlock = selectBlockIndex;
        selectBlockIndex = -1;

        // 策略：先從當前位置附近的索引開始找，如果沒找到，則進行線性掃描
        // 因為列表已經依據原理圖索引排序，空間上通常是連續的
        for (let i = currentPaletteIndexLowerBound; i < currentPaletteBlocksIndexs.length; i++) {
            if (currentPaletteBlocksState[i] === 0 && Block_In_CD.indexOf(i) === -1) {
                let fNextRpos = targetSch.vec3(currentPaletteBlocksIndexs[i]);
...
async function model_building_build(task, bot, cfg, project) {
    ...
    buildLoop: while (build_cache.currentLayer < build_cache.totalLayer) {// targetSch.palette.length
        ...
        if (!wheatherGetPalette) {
            ...
            log(bot, false, 'INFO', `\x1b[32m當前材料:\x1b[0m ${targetSch.palette[sch_palette_order[build_cache.currentPalette]].Name} ${build_cache.currentPalette + 1}/${(Object.keys(layerPaletteCountByIndex)).length}`)
                        ...
                        if (checkBlockID == 1) {
                            ...
                            if (hold == null || hold.name != currentPalette.Name) {
                                ...
                                if (findMaterialSlot == -1) { //背包也沒有 回去補
                                    ...
                                    await sleep(80)
                                    bot.updateHeldItem();
                                    if (!bot.heldItem || bot.heldItem.name !== currentPalette.Name) {
                                        log(bot, false, 'WARN', '切換後手上仍無材料，本輪跳過並重試')
                                        continue;
                                    }
                                }
        ...
        const currentBlocks = layerPaletteBlocksByIndex[sch_palette_order[build_cache.currentPalette]];
        for (let i = currentPaletteIndexLowerBound; i < currentBlocks.length; i++) {
            if (currentPaletteBlocksState[i] === 0 && blockInCD.indexOf(i) === -1) {
                let fNextRpos = targetSch.vec3(currentBlocks[i]);
                let fNextApos = fNextRpos.plus(build_cache.placement_origin);
