...
class MapartPrinter extends BasePrinter {
    ...
    async build(task, bot, cfg, project, sharedState) {
        ...
        for (let i = 0; i < targetSch.Metadata.TotalVolume; i++) {
            ...
            if (p != 0) {
                // 如果有區域限制，檢查方塊是否在區域內 (使用絕對座標)
                if (cfg.workRegion) {
                    const relativePos = targetSch.vec3(i);
                    const absoluteX = relativePos.x + cfg.schematic.placementPoint_x;
                    const absoluteZ = relativePos.z + cfg.schematic.placementPoint_z;
                    
                    if (absoluteX < cfg.workRegion.minX || absoluteX > cfg.workRegion.maxX ||
                        absoluteZ < cfg.workRegion.minZ || absoluteZ > cfg.workRegion.maxZ) {
        ...
        try {
            while (build_cache.currentPalette < targetSch.palette.length) {
                ...
                if (!wheatherGetPalette) {
                    ...
                    for (let i = 0; i < IgnoreAirArray.length; i++) {
                        // 分工邏輯：只處理屬於自己編號的方塊
                        if (cfg.worker_count > 1 && (i % cfg.worker_count !== cfg.worker_id)) continue;
                ...
                for (let [chunkKey, indices] of spatialHash.entries()) {
                    ...
                    for (let i = 0; i < indices.length; i++) {
                        ...
                        // 分工邏輯：使用 IgnoreAirArray 中的索引進行一致的分工
                        if (cfg.worker_count > 1) {
                            const globalIdx = currentPaletteBlocksIndexs[idx];
                            const ignoreAirIdx = globalToIgnoreAirIndex.get(globalIdx);
                            if (ignoreAirIdx === undefined || (ignoreAirIdx % cfg.worker_count !== cfg.worker_id)) continue;
                        }
                ...
                for (let cP_dz = -4; cP_dz <= 4; cP_dz++) {
                    for (let cP_dx = -4; cP_dx <= 4; cP_dx++) {
                        for (let cP_dy = 5; cP_dy >= -3; cP_dy--) {
                            ...
                            // 分工邏輯：使用 IgnoreAirArray 中的索引進行一致的分工
                            if (cfg.worker_count > 1) {
                                const ignoreAirIdx = globalToIgnoreAirIndex.get(currentPosPlace_d_Index);
                                if (ignoreAirIdx === undefined || (ignoreAirIdx % cfg.worker_count !== cfg.worker_id)) continue;
                            }
