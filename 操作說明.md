...
| `moneyTransferTarget` | 選填，轉帳目標玩家 ID（轉帳功能需程式內有排程才會執行） |
...
| `job` | 執行 `/job`，開啟職業選單後自動點選「礦工」(slot 19)，再點確認 (slot 40)。 |
| `gorpg` | 執行 `/rpg`，開啟選單後點選 RPG-1 (slot 9)，再點確認 (slot 24)。 |
...
5. **轉帳與每日獎勵**  
   `transferMoneyTask`、`getMoney` 等函式已實作，但目前 **未在程式內排程**（每小時轉帳、每日領獎未自動啟動）。若需要定時轉帳或每日領獎，需在 `startBot` 的 `spawn` 或適當位置自行加入 `setInterval` / `setTimeout` 呼叫。
