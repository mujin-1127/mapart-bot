    ...
    setup() {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('deploy_task', async ({ filename, pos, region, replaceMaterials, botIds }) => {
                ...
                const totalWidth = region.maxX - region.minX;
                const widthPerBot = Math.floor(totalWidth / count);

                for (let i = 0; i < count; i++) {
                    const inst = participants[i];
                    
                    // 以指定的總區域為基準進行切分
                    const minX = region.minX + (i * widthPerBot);
                    const maxX = (i === count - 1) ? region.maxX : region.minX + ((i + 1) * widthPerBot);
