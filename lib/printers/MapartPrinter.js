    ...
    async build(task, bot, cfg, project, sharedState) {
        ...
        try {
            ...
            if (missingIndices.length > 0) {
                this.log(bot, true, "WARN", `校對完畢：發現 ${missingIndices.length} 個漏蓋方塊，正在補救...`);
                // 重新執行一次簡化的建造邏輯處理這些漏掉的方塊
                // 為了簡單起見，我們直接重置部分狀態並讓循環針對這些點位再跑一次
                // 但為了不破壞原本的 build_cache，我們採用專門的補救邏輯
                for (const bIdx of missingIndices) {
                    ...
                    if (!bot.heldItem || bot.heldItem.name !== expectedName) {
                        ...
                        if (invItem) {
                            ...
                            // 若真的沒材料了，去補給站 (雖然最終檢查通常剩很少，但保險起見)
                ...
                this.log(bot, true, "INFO", "漏蓋方塊已補齊。");
            } else {
