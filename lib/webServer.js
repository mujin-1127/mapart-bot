...
class WebServer extends EventEmitter {
            ...
            socket.on('deploy_task', async ({ filename, pos, region, replaceMaterials, botIds }) => {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    if (inst.bot && inst.bot.sharedState) {
                        inst.bot.sharedState.build_cache = { hash: "" }; // 重置緩存 hash 以強制重新初始化
                    }

                    // 2. 寫入實體設定檔並清除舊進度
                    const configPath = path.join(process.cwd(), 'config', inst.id, 'mapart.json');
                    const cachePath = path.join(process.cwd(), 'config', inst.id, 'build_cache.json');
                    try {
                        // 確保目錄存在
                        ...
                        // 清除舊進度檔案，確保新任務從頭開始
                        if (existsSync(cachePath)) await fs.unlink(cachePath);
                        
                        logger.info(`已部署任務給 ${inst.id} (區域 X: ${region.minX}~${region.maxX})`);
                    } catch (e) {
    ...
    getBotStatus(instance) {
        ...
        const bc = bot?.sharedState?.build_cache || instance.sharedState?.build_cache || null;
