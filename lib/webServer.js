...
class WebServer extends EventEmitter {
    constructor(bot, port = 3000, configPath = '') {
        super();
        this.bot = bot;
        this.port = port;
    ...
    setup() {
        // ... (existing code for static files, etc.)
        // 靜態檔案目錄 (前端介面)
        ...
        this.app.get('/api/accounts', async (req, res) => {
            try {
                const data = await readConfig(this.configPath);
                res.json(data.bots || []);
            } catch (err) {
        ...
        this.app.post('/api/accounts', async (req, res) => {
            try {
                const globalConfig = await readConfig(this.configPath);
                globalConfig.bots = req.body;
                await saveConfig(this.configPath, globalConfig);
        ...
        this.app.get('/api/config/:type', async (req, res) => {
            ...
            try {
                ...
                res.status(200).json({ error: '設定檔尚未建立，請等候機器人進入遊戲。' });
            }
        });
        ...
        // ... (existing code for file-list, select-file, etc.)
        // API: 檔案列表 (自訂 UI 使用)
            ...
            socket.on('command', async (cmd) => {
                ...
                if (this.onCommandHandler) {
                    try {
                        ...
                        logger.error(`指令處理器執行錯誤: ${e.message}`);
                    }
                }
                
                // 否則當作一般聊天發送 (增加安全性檢查)
                if (this.bot && this.bot._client && this.bot.chat) {
                    try {
                        this.bot.chat(cmd);
                    } catch (e) {
                        logger.error(`發送指令失敗: ${e.message}`);
                    }
                } else {
                    logger.warn('機器人尚未完全連線，無法發送指令');
                }
            });
