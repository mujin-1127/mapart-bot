const { sleep, readConfig, v, promiseWithTimeout } = require('../lib/utils');
...
const station = {
    ...
    /** 回傳同材料的所有箱子在 materials 裡的索引陣列，空則 [] */
    getIndicesOF: function (stationConfig, target) {
        const out = []
        for (let i = 0; i < stationConfig.materials.length; i++) {
            if (stationConfig.materials[i][0] === target) out.push(i)
        }
        return out
    },
            ...
            if (mt_cfg != -1) {  //取出 或放回原和
                if (quantity >= 0) {
                    ...
                    for (let boxIdx = 0; boxIdx < materialIndices.length && remain > 0; boxIdx++) {
                        const cfg = materialIndices[boxIdx];
                        let boxPos = new Vec3(stationConfig.materials[cfg][1][0], stationConfig.materials[cfg][1][1], stationConfig.materials[cfg][1][2]);
                        let standOffest = v(stationConfig.offset[stationConfig.materials[cfg][1][3]]);
                        let btnOffset = v(stationConfig.offset[stationConfig.materials[cfg][1][4]]);
                        ...
                        q: while (trys++ < 7) {
                            ...
                            if (!shulker_box) {
                                console.log(`開啟盒子 ${restockid} 失敗`, boxPos);
                                continue q;
                            }
    ...
    oldrestock: async function (bot, stationConfig, RS_obj_array) {
        ...
        async function st_restock_single(stationConfig, restockid, quantity) {
            ...
            if (stand_dirc_offset == undefined || botton_dirc_offset == undefined) {
                ...
                並確保該偏移值確實存在於offset中`)
                throw new Error('材料站設定錯誤: 站點與按鈕偏移未定義，請檢查 station 的 materials 與 offset')
            ...
            if (quantity == -1) {
                ...
                while (maxTryTime++ < 5) {
                    ...
                    try {
                        shu = await promiseWithTimeout(bot.openBlock(bot.blockAt(shulkerBox_loc)), 1000);
                        success_open = true;
                        console.log("開盒子成功");
                        break;
                    } catch (e) {
                        console.log("開盒子失敗", e.message || e);
                        await sleep(100);
                    }
                }
                if (success_open) {
                ...
                if (remain > 0 || remain == -1) {
                    ...
                    while (maxTryTime++ < 5) {
                        try {
                            ...
                            shu = await promiseWithTimeout(bot.openBlock(bl), 1000);
                            success_open = true;
                            console.log("開盒子成功");
                            break;
                        } catch (e) {
